      ...
      <main className="relative z-20 flex w-full items-center justify-center px-4 py-12">
        ...
        >
          ...
          <section className="rounded-[28px] border-[1.5px] border-[rgba(255,198,218,0.6)] bg-[rgba(255,255,255,0.85)] p-[clamp(28px,5vw,44px)] shadow-[0_20px_60px_rgba(244,143,177,0.2),0_4px_20px_rgba(0,0,0,0.05)] backdrop-blur-[24px]">
            <form onSubmit={handleSubmit} noValidate aria-label="로그인 폼" className="flex flex-col gap-5">
              {isError && (
                <output role="alert" aria-live="polite" className="flex items-center gap-2.5 rounded-xl border border-[#ffcdd2] bg-[#fff0f3] px-4 py-3 text-sm font-medium text-[#c62828]">
                  <svg aria-hidden="true" className="h-4 w-4 shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              <fieldset className="m-0 flex flex-col gap-1.5 border-none p-0">
                <label htmlFor="email" className="text-sm font-bold text-[#7b2d52]">이메일</label>
                <div className="group relative">
                  <input
                    ...
                    className={`box-border w-full rounded-xl bg-[#fff9fb] py-[13px] pl-4 pr-11 text-[15px] text-[#4a2030] outline-none transition-all ${emailError ? "border-[1.5px] border-[#f44336]" : "border-[1.5px] border-[#f9c6d0]"}`}
                  />
                  <button
                    type="button"
                    onClick={() => {
                ...
                {emailError && (
                  <small id="email-error" role="alert" className="text-xs font-medium text-[#e53935]">
                    {emailError}
                  </small>
                )}
              </fieldset>
              ...
              <fieldset className="m-0 flex flex-col gap-1.5 border-none p-0">
                <label htmlFor="password" className="text-sm font-bold text-[#7b2d52]">비밀번호</label>
                <div className="group relative">
                  <input
                    ...
                    className={`box-border w-full rounded-xl bg-[#fff9fb] py-[13px] pl-4 pr-20 text-[15px] text-[#4a2030] outline-none transition-all ${passwordError ? "border-[1.5px] border-[#f44336]" : "border-[1.5px] border-[#f9c6d0]"}`}
                  />
                  <button
                    type="button"
                    onClick={() => {
                ...
                {passwordError && (
                  <small id="password-error" role="alert" className="text-xs font-medium text-[#e53935]">
                    {passwordError}
                  </small>
                )}
              </fieldset>
              ...
              <div className="mt-1 flex flex-col gap-3">
                <button
                  ...
                  disabled={isPending}
                  aria-busy={isPending}
                  className={`w-full rounded-xl border-none px-3.5 py-[14px] text-base font-bold text-white shadow-[0_6px_20px_rgba(244,143,177,0.45)] transition-all ${isPending ? "cursor-not-allowed bg-gradient-to-br from-[#f48fb1] to-[#ce93d8] opacity-60" : "cursor-pointer bg-gradient-to-br from-[#f48fb1] to-[#ce93d8]"}`}
                >
                  {isPending ? (
                    <span className="flex items-center justify-center gap-2">
                      <svg className="inline-block animate-spin" aria-hidden="true" width="18" height="18" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          ...
          <footer className="mt-6 text-center">
            <small className="text-[13px] text-[#c084a0]">오늘도 좋은 하루 되세요 🌷</small>
          </footer>
        </article>
      </main>
    </>
